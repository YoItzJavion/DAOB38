...
const roleCommand = ',muterole' // For servers to configure the mute role ID


// --- Set mute role command ---

// Parse a command like: !mute-role <@role>
let [argsRole, infoOrErrorRole] = prefix(roleCommand, `,muterole <@role>`)
  .role('Role')
  .parse(content);
...
const [argsMute, infoOrErrorMute] = prefix(muteCommand, `,mute <@user> <reason>.`)
  .user('User')
  .text('Reason')
  .parse(content);
...
const [argsUnmute, infoOrErrorUnmute] = prefix(unmuteCommand, `,unmute <@user>`)
  .user('User')
  .parse(content);
